...
create table t_user (
  ...
  constraint pk_t_user primary key (email))
;
...
alter table image add constraint fk_image_user_2 foreign key (user_email) references t_user (email) on delete restrict on update restrict;
...
alter table item add constraint fk_item_user_4 foreign key (user_email) references t_user (email) on delete restrict on update restrict;
create index ix_item_user_4 on item (user_email);
alter table t_user add constraint fk_t_user_address_5 foreign key (address_internal_id) references address (internal_id) on delete restrict on update restrict;
create index ix_t_user_address_5 on t_user (address_internal_id);
